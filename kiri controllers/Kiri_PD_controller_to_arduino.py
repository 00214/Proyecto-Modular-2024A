...
# Inicializa las ganancias del controlador PD
...
def PD_controller(posicion_deseada, posicion_actual, error_anterior, Kp, Kd):
    """
    Controlador PD que calcula la salida del controlador basado en el error y su derivada.
    """
    # Calcula el error
    ...
    # Calcula la salida del controlador PD
    salida_del_controlador = Kp * error + Kd * derivada_del_error

    return salida_del_controlador, error
...
def jointStatesCallback(data):
    ...
    for i in range(8):
        ...
        # Calcula la salida del controlador PD y el error
        salida_del_controlador, error = PD_controller(posicion_deseada, posiciones_actuales[i], error_anterior[i], Kp[i], Kd[i])
